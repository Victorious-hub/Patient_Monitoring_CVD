...
class AnalysisSelector:
    @transaction.atomic
    def blood_analysis_list(self) -> Iterable[BloodAnalysis]:
        patients = BloodAnalysis.objects.all()
        return patients

    @transaction.atomic
    def blood_analysis_get(self, slug: str) -> BloodAnalysis:
    ...
    @transaction.atomic
    def doctor_patient_blood_get(self, slug: str) -> BloodAnalysis:
        blood_analysis = BloodAnalysis.objects.filter(patient__patient__slug=slug).last()
        return blood_analysis

    @transaction.atomic
    def doctor_patient_cholesterol_get(self, slug: str) -> CholesterolAnalysis:
        cholesterol_analysis = CholesterolAnalysis.objects.filter(patient__patient__slug=slug).last()
        return cholesterol_analysis

    @transaction.atomic
    def cholesterol_analysis_get(self, slug: str) -> CholesterolAnalysis:
        patient_card = get_object(PatientCard, patient__slug=slug)
    ...
    @transaction.atomic
    def patient_conclusions_get(self, slug: str) -> Iterable[Conclusion]:
        conclusion = Conclusion.objects.filter(patient__patient__slug=slug)
        return conclusion
